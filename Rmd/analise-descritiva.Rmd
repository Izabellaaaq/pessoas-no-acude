...
# Quem passou mais fora da faixa: homens ou mulheres?

```{r, message=FALSE}
pedestres %>%
  filter(local == 'burrinhos') %>%
  select(horario.inicial, horario.final, fora.da.faixa, mulher, homem) %>%
  melt(var.ids = c('horario.inicial', 'horario.final'), variable.name = 'genero', value.name = 'fora.da.faixa') %>%
  filter(genero != 'fora.da.faixa') %>%
  ajeitar.horario(numerico = F) %>%
  mutate(horario = ifelse(horario.inicial < hm('12:00'), 'Manhã', ifelse(horario.inicial < hm('18:00'), 'Tarde', 'Noite'))) %>%
  mutate(horario = factor(horario, levels = c('Manhã', 'Tarde', 'Noite', ordered = T))) %>%
  group_by(horario, genero) %>%
  summarise(fora.da.faixa = sum(fora.da.faixa)) %>%
  ungroup() %>%
  group_by(horario) %>%
  mutate(percentagem = fora.da.faixa / sum(fora.da.faixa)) %>%
  mutate(label = paste0(as.character(round(100 * percentagem, 1)), '%')) %>%
  mutate(genero = factor(genero, levels = c('mulher', 'homem'), labels = c('mulheres', 'homens'))) %>%
  ggplot(aes(x = horario, y = percentagem, fill = genero)) +
  geom_bar(stat = 'identity', position = 'stack') +
  geom_text(aes(y = rep(c(0.8, 0.3), 3), label = label), color = 'white', alpha = 0.9, fontface = 'bold') +
  scale_y_continuous(labels = percent_format()) +
  labs(title = 'Quem passou mais fora da faixa?',
       subtitle = 'Percentagem de homens e mulheres que atravessaram fora da faixa ao longo do dia',
       x = 'Horário',
       y = 'Percentagem',
       fill = 'Gênero') +
  coord_flip()

ggsave('../graficos/quem-atravessou-fora-da-faixa.png', height = 5.5, width = 8.5)
```
